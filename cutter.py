...
def rotate_box_dot(x_cen, y_cen, width, height, theta):
    ...
    # answer_ = np.array([rotated_x1, rotated_y1, rotated_x2, rotated_y2, rotated_x3, rotated_y3, rotated_x4, rotated_y4])

    return answer_dict_

def roi_in_box(img_width, img_height, rbox_dict, width_pad, height_pad):
    min_x = np.min(rbox_dict['Rx'])
    min_y = np.min(rbox_dict['Ry'])
    max_x = np.max(rbox_dict['Rx'])
    max_y = np.max(rbox_dict['Ry'])
...
def cutting_test(config):
    ...
    for frm in frame_list:
        ...
        for trk in frame_in_trk_list:
            ...
            if roi_in_box(img_w, img_h, Rdot, config['sample_img']['width_roi_padding'], config['sample_img']['height_roi_padding']):
                ## rotated_box
                if config['sample_img']['show_option']:
                    ...
                    img = cv2.rectangle(img, (np.min(Rdot['Rx']), np.min(Rdot['Ry'])), (np.max(Rdot['Rx']), np.max(Rdot['Ry'])), (255,0,0), 2 )
...
if __name__ == '__main__':
    with open('cutting_config.yaml') as file:
